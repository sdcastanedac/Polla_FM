...
# PALETA DE COLORES INSTITUCIONAL Y HOMOGÉNEA
...
# Estilos CSS Unificados (Fuerza el modo claro en toda la interfaz)
st.markdown(f"""
    <style>
    ...
    /* Estilo para las tarjetas del Podio */
    ...
    /* --- ESTILO DE LA TABLA HTML COMPATIBLE Y HOMOGÉNEA --- */
    .tabla-contenedor {{
    ...
    /* Resalte suave para la fila del líder actual */
...
if df is not None:
    ...
    if len(df) >= 1:
        col2.metric("🥇 1er Puesto", df.iloc[0]['Participante'], f"{int(df.iloc[0][columna_orden])} Puntos")
    if len(df) >= 2:
        col1.metric("🥈 2do Puesto", df.iloc[1]['Participante'], f"{int(df.iloc[1][columna_orden])} Puntos")
    if len(df) >= 3:
        col3.metric("🥉 3er Puesto", df.iloc[2]['Participante'], f"{int(df.iloc[2][columna_orden])} Puntos")
        
    st.markdown("<br>", unsafe_allow_html=True)

    # Distribución simétrica: Gráfica a la izquierda, Tabla a la derecha
    ...
    with col_tabla:
        ...
        # Generar la estructura de la tabla en HTML puro para blindar los colores de fondo
        html_tabla = f"<div class='tabla-contenedor'><table class='tabla-polla'>"
        html_tabla += "<thead><tr>"
        ...
        for _, row in df.iterrows():
            ...
            for col in df.columns:
                val = row[col]
                # Si el valor es numérico flotante, lo convertimos a entero para presentación limpia
